...
<head>
    ...
    <title>FORM SỬA LOẠI DỊCH VỤ</title>
...
//
$sql_tim="SELECT * FROM t_loaidv WHERE maloaidv = '".$maloaidv."'";
$result=$conn->query($sql_tim);
//đọc dữ liệu
while ($row = $result->fetch_assoc())
{
  $maloaidv=$row['maloaidv'];
  $tenloaidv=$row['tenloaidv'];
  $madv=$row['madv'];
}
?>
<form action="sualdichvu.php" method="POST">
   <center>
       <table>
        <h2>SỬA THÔNG TIN LOẠI DỊCH VỤ</h2>
            <tr>
              <td>
                Mã loại dịch vụ
              </td>
            ...
            <tr>
              ...
              <td><input type="text" name="txttenloaidv" value="<?php echo $tenloaidv;?>"></td>
            </tr>
            <tr>
              <td>
                Mã dịch vụ
